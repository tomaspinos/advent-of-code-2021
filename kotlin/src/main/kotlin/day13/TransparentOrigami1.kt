...
fun main() {
    println(TransparentOrigami1().fold(Common.readInput("/day13/input.txt")))
}

class TransparentOrigami1 {

    fun fold(input: List<String>): Int {
        val paperAndFolds = readInput(input)

        paperAndFolds.paper.print()

        val paper = paperAndFolds.paper.fold(paperAndFolds.folds.first())
        paper.print()
    ...
    class Paper(private val dots: Array<BooleanArray>) {
        ...
        fun dotCount(): Int {
            return dots.flatMap { column -> column.asList() }.count { dot -> dot }
        }
